...
start = time.time()
while(1):
	comp_string = ''.join(random.choice(string.printable) for _ in range(32))
	crc_value = crc(comp_string)
	if(std_crc == crc_value):
		end = time.time()
		print counter, "different strings checked.\nThe CRC32 Value:", crc_value, "\n The two strings that returned the collision:", comp_string, ", ", std_number
		print "---- %s seconds ----" %(end - start)
		email_finish((end - start), comp_string )
		break
	else:
		if(counter % 100000 == 0):
